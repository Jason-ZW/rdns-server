...
func optionCompleter(args []string, long bool) []prompt.Suggest {
	...
	if l <= 1 {
		if long {
			return prompt.FilterHasPrefix(flagGlobal, "--", false)
		}
		return flagGlobal
	}
		...
		if len(commandArgs) > 1 && len(command.Subcommands) > 0 {
			for _, sub := range command.Subcommands {
				if sub.Name == commandArgs[1] {
					suggests = getFlagsSuggests(sub)
					break
				}
			}
		} else {
			suggests = getFlagsSuggests(command)
		}
	}
