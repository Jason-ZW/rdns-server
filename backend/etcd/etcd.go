...
func (b *Backend) Set(opts *model.DomainOptions) (d model.Domain, err error) {
	...
	if err := b.setFrozen(opts); err != nil {
		return d, err
	}
...
func (b *Backend) setRecordA(path string, opts *model.DomainOptions, exist bool) (d model.Domain, err error) {
	...
	cancel()
	if err != nil {
		return d, errors.Wrapf(err, "Failed to set lease for %s", path)
	}
	...
	// sync records
	if err := b.syncRecords(opts.Hosts, hosts, path, lease.ID); err != nil {
		return d, errors.Wrapf(err, "Failed to sync keys under the path: %s", path)
	}

	d.Fqdn = opts.Fqdn
	d.Hosts = opts.Hosts
	duration, _ := time.ParseDuration(fmt.Sprintf("%ds", lease.TTL))
	e := time.Now().Add(duration)
	d.Expiration = &e
...
func (b *Backend) setToken(opts *model.DomainOptions, exist bool) error {
	...
	if exist {
		...
		cancel()
		if err != nil || resp.Count <= 0 {
			return errors.Wrapf(err, "Not found previous key: %s", path)
		}
	...
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	_, err := b.ClientV3.Put(ctx, path, token)
	cancel()

	return errors.Wrapf(err, "Failed to set key: %s", path)
}
...
func (b *Backend) setFrozen(opts *model.DomainOptions) error {
	...
	ctx, cancel = context.WithTimeout(context.Background(), 10*time.Second)
	_, err = b.ClientV3.Put(ctx, path, "", clientv3.WithLease(lease.ID))
	cancel()
	if err != nil {
		return errors.Wrapf(err, "Failed to set key %s with lease %d", path, lease.ID)
	}
