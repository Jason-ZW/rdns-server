...
func (d *Domain) String() string {
	if d.Text != "" {
		return fmt.Sprintf("{Fqdn: %s, Text: %s, Expiration: %s}", d.Fqdn, d.Text, d.Expiration.Format(time.RFC3339))
	}
	if len(d.SubDomain) > 0 {
		return fmt.Sprintf("{Fqdn: %s, Hosts: %s, SubDomain: %s, Expiration: %s}", d.Fqdn, d.Hosts, mapToString(d.SubDomain), d.Expiration.Format(time.RFC3339))
	}
	return fmt.Sprintf("{Fqdn: %s, Hosts: %s, Expiration: %s}", d.Fqdn, d.Hosts, d.Expiration.Format(time.RFC3339))
}
